...
class VectorMemory:
    ...
    def add_interaction(self, user_input: str, response: str, task_hint: str = "general"):
        """Embeds and stores a full interaction cycle."""
        try:
            doc_id = f"int_{os.urandom(4).hex()}"
            ...
            # logger.info(f"Chronos: Interaction indexed [{doc_id}]")
        except Exception as e:
            logger.error(f"Chronos Error (add): {e}")

    def add_knowledge(self, source: str, content: str, metadata: Optional[Dict] = None):
        """Stores snippets of knowledge (files, documentation, etc)."""
        try:
            doc_id = f"knw_{os.urandom(4).hex()}"
            meta = metadata or {}
            meta["source"] = source
