...
def test_config() -> None:
    ...
    if or_key:
        print(f"[OK] OPENROUTER_API_KEY found in environment: {_mask_key(or_key)}")
    else:
...
def test_generation() -> None:
    ...
    try:
        ...
        if not result or "[Error LLM]" in str(result):
            print(f"[FAIL] Generation failed or returned empty: {result}")
        else:
